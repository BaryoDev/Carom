...
namespace Carom.ApiApproval.Tests;

/// <summary>
/// Pins the public surface of every published package.
///
/// Carom invites contributions, and a library that invites strangers without a machine-checked
/// public surface can have a well meant first pull request break every consumer with all other
/// checks green. That is what this exists to stop.
///
...
public class PublicApiTests
{
    ...
    {
        new object[] { "Carom" },
        new object[] { "Carom.AspNetCore" },
        new object[] { "Carom.DependencyInjection" },
        new object[] { "Carom.EntityFramework" },
        new object[] { "Carom.Extensions" },
        new object[] { "Carom.Http" },
        new object[] { "Carom.Telemetry.OpenTelemetry" }
    };
    ...
    private static string ApprovedPathFor(string package)
    {
        ...
        while (directory is not null && !File.Exists(Path.Combine(directory.FullName, "Carom.ApiApproval.Tests.csproj")))
        {
